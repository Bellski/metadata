...
public class QueryClass {
    ...
    private void initFields() {
        queryClassSource.addField().setPrivate().setType(DataSource.class).setName("dataSource");

        queryClassSource.addField().setPrivate().setType(String.class).setName("query").setStringInitializer(query);
    }

    private void initConstructors() {
        queryClassSource.addMethod().setPrivate().setName(name).setConstructor(true).setBody("");

        queryClassSource.addMethod().setPrivate().setName(name).setConstructor(true).setBody("this.dataSource = dataSource;").addParameter(DataSource.class, "dataSource");
    }

    private void initCreateMethods(ParameterInterface parameterInterface) {
        queryClassSource.addMethod().setPublic().setStatic(true).setName("create").setReturnType(parameterInterface.getType()).setBody("return new " + name + "();");

        queryClassSource.addMethod().setPublic().setStatic(true).setName("create").setReturnType(parameterInterface.getType()).setBody("return new " + name + "(dataSource);").addParameter(DataSource.class, "dataSource");
    }
    ...
    private void implementsParameterInterfaces(List<ParameterInterface> parameterInterfaces) {
        for (int i = 0; i < parameterInterfaces.size(); i++) {
            ...
            queryClassSource.getMethod(parameterInterface.getParamSetter().getName(), parameterInterface.getParamSetter().getParameter()).setBody("parameters[" + i + "] = value; \n" + "return this;");
        }
    }

    private void buildParameterField(int size) {
        queryClassSource.addField().setType(Object[].class).setName("parameters").setLiteralInitializer(" new Object[" + size + "];");
    ...
    public QueryClass addExecutionStepInterface(ExecutionStepInterface executionStepInterface) {
        ...
        for (int i = 0; i < parameterInterfaces.size(); i++) {
            setParamsBuilder.append("pstm.setObject(").append(i + 1).append(",").append("parameters[").append(i).append("]);").append("\n");
        }

        queryClassSource.getMethod("execute", Connection.class).setBody("final PreparedStatement pstm = connection.prepareStatement(query); \n" + setParamsBuilder.toString() + "\n" + "ResultSet resultSet = pstm.executeQuery(); \n"
    ...
    public void save(VirtualFile directoryFile, Project project) {
        ...
        VirtualFile metaSqlPackage = directoryFile.findFileByRelativePath("target/generated-sources/meta/" + packageName.replaceAll("\\.", "/"));
            ...
            ApplicationManager.getApplication().runWriteAction(new Runnable() {
                ...
                public void run() {
                    ...
                    if (queryClassFile == null) {
                        psiManager.findDirectory(queryDir).add(psiFileFactory.createFileFromText(name + ".java", JavaFileType.INSTANCE, normalizeClassSource(queryClassSource.toString())));
                    }
                    ...
                    for (ParameterInterface parameterInterface : parameterInterfaces) {
                        ...
                        if (parameterInterfaceFile == null) {
                            psiManager.findDirectory(queryDir).add(psiFileFactory.createFileFromText(parameterInterface.getJavaFileName(), JavaFileType.INSTANCE, normalizeClassSource(parameterInterface.toString())));
                        }
                    }
                    ...
                    if (executionStepInterfaceFile == null) {
                        psiManager.findDirectory(queryDir).add(psiFileFactory.createFileFromText(executionStepInterface.getFileName(), JavaFileType.INSTANCE, normalizeClassSource(executionStepInterface.toString())));
                    }
                }
            });
        }
