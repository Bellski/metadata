...
import ru.bellski.metasql.lang.psi.MetaSqlParam;
...
public class SqlParamFolding extends FoldingBuilderEx {
    ...
    public FoldingDescriptor[] buildFoldRegions(@NotNull PsiElement root, @NotNull Document document, boolean quick) {
        ...
        Collection<MetaSqlParam> metaParams = PsiTreeUtil.findChildrenOfType(InjectedLanguageUtil.findInjectedFile(root.getChildren()[0], MetaSqlFile.class), MetaSqlParam.class);
        ...
        MetaSqlParam[] metaParamsArray = metaParams.toArray(new MetaSqlParam[metaParams.size()]);

        for (int i = 0; i < metaParamsArray.length; i++) {
            if (i < paramsArray.length) {
                SqlParameter sqlParam = paramsArray[i];
                final MetaSqlParam metaSqlParam = metaParamsArray[i];
