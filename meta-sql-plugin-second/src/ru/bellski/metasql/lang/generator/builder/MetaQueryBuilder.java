...
public class MetaQueryBuilder {
    ...
    private String resolveImport() {
        return importPackages.stream().map(packageName -> "import " + packageName).collect(Collectors.joining("; \n"));
    }

    private String joinSteps() {
        String steps = stepBuilders.stream().map(StepBuilder::getName).collect(Collectors.joining(", "));
    ...
    private String implementSetters() {
        return stepBuilders.stream().map(StepBuilder::getOverrideSetter).collect(Collectors.joining("\n\n"));
    }
