...
WITH params AS ( SELECT
	...
	claim_stuff AS (
		SELECT
			...
			,ins_type.name                                 AS insuranceType
			...
			,CASE car.custom
				WHEN NULL THEN    ARRAY[null::text,       null::text,     null::text            , null::text   ]
				WHEN 'undef' THEN ARRAY[null::text,       null::text,     null::text            , null::text   ]
				WHEN 'concern' THEN (
...
SELECT
	...
	,car_model_array[4] AS ts_type --todo-vs проверить, правильно ли ts_type берется? добавил, потому что не работал setVehicleType (Cas1DbManager.java:521, The column name ts_type was not found in this ResultSet.)
