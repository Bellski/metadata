...
public class SlotImplementationGenerator extends Generator {
    ...
    public PsiMethod generate(SlotImplementationModel slotImplementationModel, PsiElementFactory elementFactory) throws IOException, TemplateException {
        final Template template = freeMarkerConfig.getTemplate("ProvidesSlotMethod.ftl");

        try (Writer out = new StringWriter()) {
            template.process(slotImplementationModel, out);
            return elementFactory.createMethodFromText(out.toString().trim(), null);
        }
    }
}
